...
class Repository {
  constructor(dbConnection, registry) {
    ...
    this.dbConnection = typeof dbConnection === "string" ? Connection.get(dbConnection, reg) : dbConnection;
  }
  ...
  async upsert(data, updatedKeys, options = {}) {
    const values = await this.beforeInsert({ ...data });
    return this.dbTable.insertOnDuplicateUpdate(values, updatedKeys, options.connection);
  }
  ...
  async update(condition, data, options = {}) {
    ...
    const result = await this.dbTable.update(values, condition, options.connection);
