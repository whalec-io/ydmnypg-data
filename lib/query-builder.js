class QueryBuilder {
  ...
  build() {
    ...
    const selectCols =
      this.selects.length > 0
        ? this.selects.map(c => (c === "*" || c.includes(".") || c.includes(" ") ? c : this.col(c))).join(", ")
        : "*";
    ...
    if (this.orderBys.length > 0) sql += ` ORDER BY ${this.orderBys.join(", ")}`;
  ...
  async count(adapter, connection) {
    ...
    const row = Array.isArray(result) ? result[0] : result.rows ? result.rows[0] : result[0];
