...
class MigrationRunner {
  ...
  async recordApplied(migration, connection) {
    ...
    await this.adapter.executeQuery(sql, [migration.version, migration.description || ""], connection);
  }
  ...
  async migrate(target = "latest") {
    return this.adapter.task(async connection => {
      ...
      const pending = this.migrations.filter(m => !applied.includes(m.version) && m.version <= maxVersion);
      ...
      for (const migration of pending) {
        try {
          ...
          throw new MigrationError(`Migration v${migration.version} failed: ${e.message}`, e);
        }
      }
    ...
    return this.adapter.task(async connection => {
      ...
      for (const version of toRollback) {
        ...
        try {
          ...
          console.log(`[migration] rolled back v${migration.version}: ${migration.description || ""}`);
          rolledBack.push(version);
        } catch (e) {
          throw new MigrationError(`Rollback v${version} failed: ${e.message}`, e);
        }
      }
