...
class BaseTableAdapter {
  ...
  retryWhenReady(methodName, args, delayMs = 100, timeoutMs = 10000) {
    this.adapter.logger.warn(`Table '${this.tableName}' not ready — delaying ${methodName}...`);
    return new Promise((resolve, reject) => {
      ...
      const attempt = () => {
        if (this.ready) {
          ...
          reject(new Error(`Table '${this.tableName}' not ready after ${timeoutMs}ms — ${methodName} timed out`));
        } else {
  ...
  applyDefaults(action, values, touchedFields = []) {
    for (const [key, def] of Object.entries(this.attributes)) {
      if (!def) continue;
      if (def.json === true) {
        if (action === QueryAction.INSERT && (!values[key] || values[key] === "")) {
          values[key] = this.toJSON(values[key], "{}");
        } else if (values[key] !== undefined && typeof values[key] === "object") {
          values[key] = this.toJSON(values[key], "{}");
        }
        ...
        } else if (typeof def.triggerKey === "string" && values[def.triggerKey] !== undefined) {
          values[key] = this.generateTimestamp();
        } else if (action === QueryAction.INSERT && (key === "createdAt" || key === "updatedAt")) {
  ...
  setAttributes(attributes) {
    for (const [fieldName, attribute] of Object.entries(attributes)) {
      ...
      if (attribute.primaryKey === true) {
        ...
        this.useAutoIncrement = attribute.autoIncrement === true || attribute.autoIncrement === "auto";
      }
    }
  ...
  async createTable(connection) {
    if (this.creating) {
      this.adapter.logger.warn(`createTable '${this.tableName}' ignored — already creating`);
      return false;
    }
    if (this.tableName === "*") throw new Error("Wildcard table does not support DDL");
    this.creating = true;
    try {
  ...
  async dropTable(connection) {
    if (this.dropping) {
      this.adapter.logger.warn(`dropTable '${this.tableName}' ignored — already dropping`);
      return false;
    }
    if (this.tableName === "*") throw new Error("Wildcard table does not support DDL");
    this.dropping = true;
  ...
  syncTable(connection) {
    if (this.tableName === "*") return Promise.reject(new Error("Wildcard table does not support DDL"));
    return this.doSyncTable(connection);
  }

  getColumns(connection) {
    if (!this.ready) return this.retryWhenReady("getColumns", [connection]);
    if (this.tableName === "*") return Promise.reject(new Error("Wildcard table"));
  ...
  async initRecords(records, connection) {
    if (!this.ready) return this.retryWhenReady("initRecords", [records, connection]);
  ...
  delete(condition, connection) {
    if (!this.ready) return this.retryWhenReady("delete", [condition, connection]);
    if (this.tableName === "*") return Promise.reject(new Error("Wildcard table"));
    return this.doDelete(condition, connection);
  }

  execute(command, params = {}, connection) {
    if (!this.ready) return this.retryWhenReady("execute", [command, params, connection]);
