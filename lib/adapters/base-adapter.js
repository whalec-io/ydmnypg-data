...
class BaseAdapter {
  ...
  startKeepAlive(intervalSeconds) {
    ...
    this.keepAliveTimer = setInterval(async () => {
      ...
      try {
        ...
        this.logger.error("keep-alive ping failed, recreating pool:", err.message);
        try {
  ...
  async preloadSchemas() {
    ...
    try {
      client = await this.getConnection();
      const tableNames = await this.fetchTableNames(this.dbConnection.database, client);
      ...
      this.preloaded = true;
      this.logger.info(`preloaded ${tableNames.length} tables from '${this.dbConnection.database}'`);
    } catch (e) {
  ...
  async initSchema(design, schemaName, schema, connection) {
    this.logger.initializing = true;
    const { mappingName, view, attributes, constraints, rowFormat, records } = schema;
    const database = this.dbConnection.database;

    if (design.dbConnection.database !== database) {
      throw new Error(`Connection database mismatch: expected '${database}', got '${design.dbConnection.database}'`);
    }
    ...
    if (view) {
      ...
      if (rowFormat && typeof ta.setRowFormat === "function") ta.setRowFormat(rowFormat.toUpperCase());
      if (attributes) ta.setAttributes(attributes);
      if (constraints) ta.setConstraints(constraints);

      if (this.autoCreate) {
        if (schema.truncate === true && ta.isCreated) await ta.dropTable(connection);
        if (!ta.isCreated) {
          await ta.createTable(connection);
          if (records && records.length > 0) await ta.initRecords(records, connection);
        } else {
  ...
  schema(tableName = "*") {
    ...
    if (!this.tableAdapters.has(key)) {
      this.tableAdapters.set(key, this.createTableAdapter(tableName, tableName === "*"));
    }
