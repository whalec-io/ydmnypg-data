function buildColumnDef(def, key, others) {
  ...
  switch (type) {
    ...
    default:
      throw new Error(`PostgreSQL: unsupported column type '${type}' on ${others.name}.${key}`);
  }
...
function buildCreateTable(tableName, attributes, constraints) {
  ...
  for (const [key, def] of Object.entries(constraints || {})) {
    if (def.type === "unique") {
      columns.push(`CONSTRAINT "${key}" UNIQUE (${def.keys.map(k => `"${k}"`).join(", ")})`);
    }
  }
  ...
  if (columns.length === 0) throw new Error(`Table '${tableName}' has no attribute definitions`);
...
function buildUpdate(tableName, values, condition) {
  ...
  return {
    ...
    values: [...valKeys.map(k => values[k]), ...condKeys.map(k => condition[k])],
  };
}
...
export { buildColumnDef, buildCreateTable, buildInsert, buildUpdate, buildDelete };
