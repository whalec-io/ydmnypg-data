...
import { buildColumnDef, buildCreateTable, buildInsert, buildUpdate, buildDelete } from "./ddl.js";

class PgTableAdapter extends BaseTableAdapter {
  async doCreateTable(connection) {
    const { sql, constraints } = buildCreateTable(this.tableName, this.attributes, this.constraints);
    ...
    for (const [key, def] of Object.entries(constraints || {})) {
      if (def.type === "btree") {
        ...
        await this.adapter.executeQuery(idxSql, [], connection).catch(e => this.adapter.logger.error(e.message));
  ...
  async doSyncTable(connection) {
    ...
    if (toAdd.length > 0) {
      ...
      for (const col of toAdd) {
        ...
        await this.adapter.executeQuery(`ALTER TABLE "${this.tableName}" ADD COLUMN ${colDef}`, [], connection);
      }
    }
  }
  ...
  async doInsertIgnore(values, connection) {
    ...
    const sql = base.replace(" RETURNING *", " ON CONFLICT DO NOTHING RETURNING *");
    const result = await this.adapter.executeQuery(sql, params, connection);
    return result.rowCount > 0 ? { ...result, affectedRows: result.rowCount } : false;
  }
  ...
  async doInsertOnDuplicateUpdate(values, updatedKeys, connection) {
    ...
    const updateClauses = updatedKeys.map(k => `"${k}" = EXCLUDED."${k}"`).join(", ");
  ...
  async doUpdate(values, condition, connection) {
    ...
    const { sql, values: params } = buildUpdate(this.tableName, values, condition);
    const result = await this.adapter.executeQuery(sql, params, connection);
  ...
  doExecute(command, params, connection) {
    ...
    return this.adapter
      .executeQuery(sql, values, connection)
