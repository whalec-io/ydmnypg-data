function buildColumnDef(q, def, key, others) {
  ...
  switch (type) {
    ...
    case "int": {
      ...
      if (others.addColumn !== true && def.autoIncrement) q += " AUTO_INCREMENT";
      break;
    }
    case "smallint": {
    ...
    default:
      throw new Error(`MySQL: unsupported column type '${type}' on ${others.name}.${key}`);
  }
...
function buildCreateTable(tableName, attributes, constraints, rowFormat) {
  ...
  if (!statement) throw new Error(`Table '${tableName}' has no attribute definitions`);
