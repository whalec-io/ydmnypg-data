...
class MysqlTableAdapter extends BaseTableAdapter {
  ...
  async doInitRecords(records, connection) {
    for (const values of records) {
      if (typeof values === "string") {
        ...
        const insert = this.buildInsertSet(values);
        await this.adapter.executeQuery(
          `INSERT INTO \`${this.tableName}\` (${insert.columns}) VALUES (${insert.placeholders})`,
          insert.params,
          connection,
        );
      }
  ...
  async doInsertIgnore(values, connection) {
    ...
    const res = await this.adapter.executeQuery(
      `INSERT IGNORE INTO \`${this.tableName}\` (${insert.columns}) VALUES (${insert.placeholders})`,
      insert.params,
      connection,
    );
  ...
  async doInsertOnDuplicateUpdate(values, updatedKeys, connection) {
    ...
    const insert = this.buildInsertSet(values);
    const update = this.buildUpdateSet(updatedValues);
    const sql = `INSERT INTO \`${this.tableName}\` (${insert.columns}) VALUES (${insert.placeholders}) ON DUPLICATE KEY UPDATE ${keyCondition}${update.assignments}`;
    const res = await this.adapter.executeQuery(sql, [...insert.params, ...update.params], connection);
  ...
  async doInsertRows(rows, connection) {
    ...
    const params = [];
    for (const row of rows) {
      for (const f of fields) params.push(row[f]);
    }
    const sql = `INSERT INTO \`${this.tableName}\` (${columns}) VALUES ${allPlaceholders}`;
    const res = await this.adapter.executeQuery(sql, params, connection);
    return res.affectedRows > 0 ? res : false;
  }
  ...
  async doInsertOnDuplicateUpdateRows(rows, condition, connection) {
    ...
    const sql = `INSERT INTO \`${this.tableName}\` (${columns}) VALUES ${allPlaceholders} ON DUPLICATE KEY UPDATE ${condition}`;
    const res = await this.adapter.executeQuery(sql, params, connection);
    return res.affectedRows > 0 ? res : false;
  }
  ...
  async doUpdate(values, condition, connection) {
    ...
    const update = this.buildUpdateSet(values);
    const where = this.buildWhereClause(condition);
    return this.adapter.executeQuery(
      `UPDATE \`${this.tableName}\` SET ${update.assignments} WHERE ${where.clause}`,
      [...update.params, ...where.values],
      connection,
    );
  }
  ...
  async doUpdateIgnore(values, condition, connection) {
    ...
    const where = this.buildWhereClause(condition);
    const res = await this.adapter.executeQuery(
      `UPDATE IGNORE \`${this.tableName}\` SET ${update.assignments} WHERE ${where.clause}`,
      [...update.params, ...where.values],
      connection,
    );
    return res.affectedRows > 0 && res.warningCount === 0 ? res : false;
  ...
  doExecute(command, params, connection) {
    ...
    const { sql, values } = mybatisMapper.getStatement(
      namespace,
      sqlId,
    ...
    return this.adapter.executeQuery(sql, values, connection);
  }
}
