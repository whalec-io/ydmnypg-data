...
class Logger {
  constructor(namespace, externalLogger) {
    ...
    if (externalLogger) {
      ...
      if (this.level === DebugLevel.NONE && (env.includes(namespace) || env.includes("dao:*"))) {
        this.level = DebugLevel.TRACE;
      }
  ...
  write(levelName, ...args) {
    if (this.ext) {
      this.ext[levelName] ? this.ext[levelName](...args) : this.ext.info(...args);
  ...
  queryStart({ sql, values }) {
    ...
    if (this.level <= DebugLevel.INFO || (!this.initializing && this.level <= DebugLevel.TRACE)) {
      const clean = (sql || "").replace(/[\n\t]+/g, " ").replace(/ +/g, " ");
      this.write("debug", "Query Start", id, "\n  ", clean, JSON.stringify(values));
    }
    return id;
  }
  ...
  queryEnd(id, meta = {}) {
    if (this.level <= DebugLevel.INFO) {
      ...
      if (meta.error) {
        this.write("error", "Query Error", id, elapsed + "ms", meta.error.message);
      } else {
  ...
  transactionStart() {
    ...
    if (this.level <= DebugLevel.DEBUG) this.write("debug", "Transaction Start", id);
    return id;
  }
